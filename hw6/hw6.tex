...
\begin{solution}
  \begin{enumerate}[label=\alph*]
    ...
    \item For each applicant $1 \leq j \leq m$, use set $B_j \in S$ represents the types of sports that the applicant $j$ is qualified for. Thus, if there exists a subset $H$ satisfies Efficient Recruiting, it is guaranteed that there are at most $k = |H|$ subsets of $S$ whose unite is equal to $S$. Since $|B_j| \leq |S|$ and $j \leq m$, the reduction is polynomial.
    \item
  \end{enumerate}
\end{solution}
\end{document}