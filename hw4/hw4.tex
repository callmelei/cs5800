...
\begin{solution}
    \begin{enumerate}
        \item The recursion to solve this problem is that for the remaining cards $A[i..j]$, the score Alice could get, denoted by $S[i,j]$, is the maximum of the two situations: 1) she picks the first card, $S[i,j] = A[i] + S[i+1,j]$; 2) she picks the last card, $S[i,j] = A[j] + S[i,j-1]$. The terminating case is that $i=j$ where $S[i,j] = A[i]$.
        \item \begin{equation*}
                  S[i,j] = \left\{
                  \begin{array}{lcl}
                      \max \{A[i] + S[i+1,j], A[j] + S[i,j-1]\} & \mbox{if} & i<j \\
                      A[i]                                      & \mbox{if} & i=j
                  \end{array}\right.
              \end{equation*}
        \item The final value to return is $S[1,n]$
